...
uint32_t PngCodec::crc32_update(uint32_t crc, const uint8_t* data, size_t len) {
...
uint32_t PngCodec::adler32(const uint8_t* data, size_t len) {
    uint32_t a = 1, b = 0;
    for (size_t i = 0; i < len; ++i) {
        a = (a + data[i]) % 65521;
        b = (b + a) % 65521;
...
void PngCodec::writeUint32BE(std::vector<uint8_t>& buffer, uint32_t value) {
...
void PngCodec::writeChunk(std::vector<uint8_t>& buffer, const char* type, const uint8_t* data, size_t len) {
    ...
    crc ^= 0xffffffff;
    writeUint32BE(buffer, crc);
}

bool PngCodec::compressData(const uint8_t* data, size_t len, std::vector<uint8_t>& compressed) {
#ifdef HAVE_ZLIB
    z_stream strm;
    strm.zalloc = Z_NULL;
    strm.zfree = Z_NULL;
...
bool PngCodec::decode(const std::string& filename, Image& image) {
    ...
    while (pos < fileSize - 12) {
        ...
        if (strcmp(chunkType, "IHDR") == 0 && chunkLen == 13) {
            ...
            image.height = (chunkData[4] << 24) | (chunkData[5] << 16) | 
    ...
    if (image.width == 0 || image.height == 0 || compressedData.empty()) {
        return false;
    }
